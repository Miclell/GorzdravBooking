...
public class ShowTimePreferencesProvider(IServiceProvider serviceProvider, ITimePreferencesService timePreferencesService) : IMenuProvider
{
    public async Task<MenuState> CreateMenuAsync(CancellationToken cancellationToken = default)
    {
        ...
        var timePreferences = await timePreferencesService.GetByUserAsync(await appSettingService.GetDefaultUserIdAsync(), cancellationToken);
        ...
        return new MenuState("Выберите пресет для удаления", items);
    }
}