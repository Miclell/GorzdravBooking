...
public class LpuMenuProvider(IExternalLpuService externalLpuService, IDataService data, IServiceProvider sp) : IMenuProvider
{
    public async Task<MenuState> CreateMenuAsync(CancellationToken ct = default)
    {
        ...
        var lpus = await externalLpuService.GetByDistrictAsync(districtId!);
        var items = lpus
