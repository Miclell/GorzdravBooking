...
public class AppointmentSlotsProvider(IAppointmentService appointmentService, IDataService data) : IMenuProvider
{
    public async Task<MenuState> CreateMenuAsync(CancellationToken ct = default)
    {
        ...
        var slots = await appointmentService.GetByDoctorAsync(lpuId, doctorId!);
        var items = slots
