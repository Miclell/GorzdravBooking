    ...
    IAppSettingsService appSettingsService) : IMenuProvider
{
    ...
    {
        var patients = await patientService.GetByUser(await appSettingsService.GetDefaultUserIdAsync(), cancellationToken);
        ...
        return new MenuState("Выберите пациента для записи", items);
    }
}